"""
Production-ready Authentication Service FastAPI Application.

Provides secure authentication, session management, and token generation
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if not api_key:
            logger.error("AUTH_SERVICE_API_KEY is required and not set")
            raise ValueError("AUTH_SERVICE_API_KEY environment variable is required")
...
async def general_exception_handler(request: Request, exc: Exception):
    ...
    logger.error("Unexpected error", error=str(exc), error_type=type(exc).__name__, path=request.url.path)
    return JSONResponse(
...
async def register(
    ...
    try:
        ...
        # Check for existing user (with lock for thread safety)
        async with db_lock:
...
async def login(
    ...
    try:
        ...
        token_data = token_manager_instance.generate_token(user["id"], user["username"])
...
async def get_current_user(request: Request) -> UserProfileResponse:
    ...
    try:
        ...
        if not user:
            logger.warning("User not found", user_id=user_id, ip=security_context.ip_address)
            raise HTTPException(
        ...
        logger.error("Failed to get user profile", error=str(e), error_type=type(e).__name__)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def update_profile(
    request: Request,
    user_update: UserRegistrationRequest
) -> UserProfileResponse:
    """
    ...
    Args:
        request: HTTP request with bearer token
        user_update: Updated user data (Pydantic validated)
    ...
    try:
        ...
        async with db_lock:
            ...
            allowed_fields = ['first_name', 'last_name', 'phone', 'currency', 'timezone', 'email']
            for field in allowed_fields:
                if hasattr(user_update, field) and getattr(user_update, field) is not None:
                    value = getattr(user_update, field)

                    if field == 'email' and any(u["email"] == value and u["id"] != user_id for u in users_db.values()):
                        logger.warning(
...
@app.post("/change-password", response_model=Dict[str, str])
async def change_password(
    ...
    Args:
        request: HTTP request with bearer token
        password_data: Old and new password (Pydantic validated)

    Returns:
        Success message

    Raises:
        HTTPException: 401 if token invalid or old password incorrect
        HTTPException: 404 if user not found
    ...
    try:
        ...
        async with db_lock:
            ...
            if not user:
                logger.warning("User not found for password change", user_id=user_id)
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
                    detail="User not found"
                )
            ...
            # Verify old password
            if not password_manager.verify_password(password_data.old_password, user["password_hash"]):
                login_tracker.record_attempt(user["username"])
                logger.warning(
        ...
        logger.warning("Password validation failed during change", error=str(e))
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e)
        )
...
async def logout(request: Request) -> Dict[str, str]:
    ...
    """
    security_context = SecurityContext(request)
    logger.info("User logged out", ip=security_context.ip_address)
