...
class InvestigationService:
    ...
    def _generate_number(self) -> str:
        self._counter += 1
        return f"INV-{datetime.now(UTC).strftime('%Y%m%d')}-{self._counter:06d}"
    ...
    async def get_investigation(self, investigation_id: UUID) -> FraudInvestigation | None:
        return self._investigations.get(investigation_id)

    async def update_status(self, investigation_id: UUID, status: InvestigationStatus) -> FraudInvestigation | None:
    ...
    async def assign_investigator(self, investigation_id: UUID, investigator: str) -> FraudInvestigation | None:
        investigation = self._investigations.get(investigation_id)
        if investigation:
    ...
    async def complete_step(self, investigation_id: UUID, step_index: int, result: str, notes: str, completed_by: str) -> FraudInvestigation | None:
        investigation = self._investigations.get(investigation_id)
    ...
    async def record_customer_contact(self, investigation_id: UUID, contact: CustomerContact) -> FraudInvestigation | None:
        investigation = self._investigations.get(investigation_id)
        if investigation:
    ...
    async def set_outcome(self, investigation_id: UUID, outcome: InvestigationOutcome, reason: str) -> FraudInvestigation | None:
        investigation = self._investigations.get(investigation_id)
        if investigation:
    ...
    async def process_refund(self, investigation_id: UUID, refund_amount: float) -> FraudInvestigation | None:
        investigation = self._investigations.get(investigation_id)
        if investigation:
