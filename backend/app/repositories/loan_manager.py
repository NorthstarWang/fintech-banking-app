...
from app.models.entities.loan_models import (
    LoanType, LoanStatus, PaymentFrequency, InterestType,
    LoanApplicationCreate, LoanApplicationResponse,
    LoanOfferResponse, LoanResponse, LoanPaymentScheduleResponse,
    LoanPaymentCreate, LoanPaymentResponse,
    LoanRefinanceAnalysis, LoanAmortizationRequest,
    LoanSummaryStats, CryptoLoanCreate
)
...
class LoanManager:
        ...
        for app in self.data_manager.loan_applications:
            if app['user_id'] == user_id:
                ...
                try:
                    ...
                    print(f"Error converting loan application {app.get('id')}: {e}")
                    continue
    ...
    def get_user_loans(self, user_id: int, status: Optional[LoanStatus] = None) -> List[LoanResponse]:
    ...
    def get_loan_details(self, loan_id: int, user_id: int) -> Optional[LoanResponse]:
    ...
    # Helper methods
    def _get_application(self, application_id: int) -> Optional[Dict[str, Any]]:
    ...
    def _get_offer(self, offer_id: int) -> Optional[Dict[str, Any]]:
    ...
    def _get_loan(self, loan_id: int) -> Optional[Dict[str, Any]]:
        """Get loan by ID."""
        for loan in self.data_manager.loans:
            if loan['id'] == loan_id:
                return loan
    ...
    def _get_scheduled_payment(self, loan_id: int, payment_number: int) -> Optional[Dict[str, Any]]:
        """Get scheduled payment info."""
        for sched in self.data_manager.loan_payment_schedules:
    ...
    def _calculate_next_payment_date(self, from_date: Optional[date] = None) -> date:
