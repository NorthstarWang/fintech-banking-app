...
from app.models.entities.loan_models import (
    LoanType, LoanStatus, PaymentFrequency,
    LoanApplicationCreate, LoanApplicationResponse,
    LoanOfferResponse, LoanResponse,
...
async def get_loan_summary_alias(
    ...
    try:
        ...
        print(f"Getting loan summary for user_id: {current_user['user_id']}")
        print(f"Total loans in system: {len(loan_manager.data_manager.loans)}")
        ...
        print(f"Active loans for user: {len(user_loans)}")
        ...
        print(f"Error in loan summary endpoint: {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()
