...
from ..models import InvoiceLineItem as InvoiceLineItemSchema
...
def categorize_for_tax(description: str, amount: float) -> TaxCategory:
    ...
    if any(word in description_lower for word in ["lunch", "dinner", "coffee", "restaurant"]):
        return TaxCategory.MEALS_ENTERTAINMENT
    if any(word in description_lower for word in ["hotel", "airbnb", "flight"]):
        return TaxCategory.TRAVEL
    if any(word in description_lower for word in ["office depot", "staples", "supplies"]):
        return TaxCategory.OFFICE_SUPPLIES
    if any(word in description_lower for word in ["internet", "phone", "electric", "water"]):
        return TaxCategory.UTILITIES
    if any(word in description_lower for word in ["rent", "lease"]):
        return TaxCategory.RENT
    if any(word in description_lower for word in ["insurance"]):
        return TaxCategory.INSURANCE
    if any(word in description_lower for word in ["laptop", "computer", "software"]):
        return TaxCategory.EQUIPMENT
    if any(word in description_lower for word in ["lawyer", "accountant", "consultant"]):
        return TaxCategory.PROFESSIONAL_SERVICES
    if any(word in description_lower for word in ["ad", "marketing", "promotion"]):
